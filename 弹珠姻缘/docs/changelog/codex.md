...
任务：统一前端为标准 9:16 竖屏游戏尺寸
分支：agent/codex/marble-cp
修改：
- 将页面外壳改为居中的 9:16 游戏框，匹配 750×1334 / 1080×1920 设计稿比例
- 超长屏不再把内容无限拉高，短屏按高度收窄
- 各主页面在游戏框内滚动或填充，弹珠盘用剩余空间自适应
- 碰撞计数展开浮层宽度跟随游戏框
验证：
- `npm test` 通过，15/15
- 停止 dev server 后执行 `npm run build` 通过
- 重启 dev server 后浏览器确认首页正常渲染
- 浏览器验证 360×640 下游戏卡片比例为 0.5625，弹珠页画布、顶部状态栏和计数条在框内正常布局
风险：
- 角色选择页内容较多，后续像素风改版时应同步压缩角色卡高度
